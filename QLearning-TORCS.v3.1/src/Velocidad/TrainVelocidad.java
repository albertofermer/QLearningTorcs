...
public class TrainVelocidad extends Controller {
	...
	ArrayList<Integer> bestSeccion = new ArrayList<>();
	int seccion = 500;
	int seccion_actual = 0;
	int tick_seccion = 0;
	boolean vuelta_es_0 = false;
	...
	public Action control(SensorModel sensors, SocketHandler mySocket) {		
		if (sensors.getLastLapTime() > 0.0) {
			train(getSpeedState(sensors), getPorcentaje(sensors), sensors, vuelta_es_0);
		...
		if(sensors.getDistanceFromStartLine() >= 0 && sensors.getDistanceFromStartLine() <= 20) {
			vuelta_es_0 = true;
		}
...
//		System.out.println("Carrera #" + indice_carreras);
		...
		if (tick >= Constantes.TICK_COMIENZO && (tick % Constantes.TICK_ENTRENAMIENTO == 0)
			...
			float[] accel_and_brake = train(getSpeedState(sensors), getPorcentaje(sensors), sensors, vuelta_es_0);
		 ...
		 * negativamente.
		 */
		if (Math.abs(sensors.getTrackPosition() - oldTrackPosition) <= 0.01 && sensors.getSpeed() <= 1) {
	...
	private void recompensar(ArrayList<float[]> recompensa, char modo, int bestTick, int oldTick) {
		System.out.println("##################");
		System.out.println("MEJOR: " + bestTick);
		System.out.println("ANTES: " + oldTick);
		Double reward = Math.abs(oldTick - bestTick)*100.0;
		...
		//Tras recompensar en un sector, se reinicia la recompensa.
		recompensa = new ArrayList<>();
	}
	...
	private float[] play(SensorModel sensors) {
		...
		System.out.println("COUNT_TICK: " + count_tick);
		System.out.println("TIKCS_ESPERA: " + Constantes.TICKS_ESPERA);
	...
	public float[] train(Integer newState, Double porcentaje, SensorModel sensors, boolean vuelta_es_0) {
...
//					getBestMoveFromTarget(newState));
			...
			if(sensors.getDistanceFromStartLine() >= seccion && vuelta_es_0) {
				System.out.println("**** SECCION " + seccion_actual + " ****");
				//Hay una seccion cada 500 ticks
				//Si tarda menos que la seccion anterior
				if(indice_carreras == 0) { //Si es la primera vuelta de todo el entrenamiento.
					System.out.println("PRIMERA VUELTA");
					bestSeccion.add(seccion_actual, tick);
					recompensar(recompensa, '+', 1, 0);
					
				}else {
					System.out.println("OTRAS VUELTAS");
					if(bestSeccion.get(seccion_actual) >= tick) {
						int oldTick = bestSeccion.get(seccion_actual);
						bestSeccion.remove(seccion_actual);
						bestSeccion.add(seccion_actual, tick);
						recompensar(recompensa, '+', tick, oldTick);
					}else
						recompensar(recompensa, '-', 0, 0);
				}
				System.out.println("Seccion actual: " + seccion_actual);
				System.out.println("Seccion siguiente: " + seccion);
				System.out.println("Porcentaje: " + porcentaje);
				System.out.println("Estado: " + getSpeedState(sensors));
