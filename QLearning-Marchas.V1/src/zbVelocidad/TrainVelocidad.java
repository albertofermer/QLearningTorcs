...
public class TrainVelocidad extends Controller {
	...
	public float[] train(Integer newState, Double porcentaje, SensorModel sensors) {
		...
		// y se reinicia.
		if (tick > Constantes.NUMERO_MAXIMO_TICKS || Math.abs(sensors.getTrackPosition()) >= Constantes.LIMITE_CARRETERA) {
			
			if(tick > Constantes.NUMERO_MAXIMO_TICKS) System.out.println("TIEMPO AGOTADO!");
			else if(Math.abs(sensors.getTrackPosition()) >= Constantes.LIMITE_CARRETERA) System.out.println("SE HA SALIDO DE LA CARRETERA!");
			...
			/**
			 * La recompensa sera una suma ponderada de:
			 * - (70%) La distancia al centro de la carretera.
			 * - (30%) La velocidad del coche.
			 */
			...
			// Se establece la recompensa para el estado anterior en funcion del estado
			// actual.
