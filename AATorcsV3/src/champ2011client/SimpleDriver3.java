...
//	}
	...
	public Action control(SensorModel sensors, SocketHandler mySocket) {
		...
		steer = train(getSteerState(sensors.getTrackPosition()), false, getPorcentaje(sensors), sensors, aprende);
		oldSteer = steer;
//	        
...
//		else if(trackPosition < - 0.6) return 4; // derecha
	...
	public float train(Integer newState, Boolean isStuck, Double porcentaje, SensorModel sensors, boolean aprende) {
		...
		if (stuck > stuckTime) {
			...
			steer = (float) (-sensors.getAngleToTrackAxis() / steerLock);
			int gear = -1; // gear R
...
//				 */
//
//				Double reward = qtable.setReward(oldState, newState, best_steer_angle, targetReward,
...
//				qtable.setReward(newState, newState, best_steer_angle, targetReward, getBestMoveFromTarget(newState));
//				qTableFrame.setQTable(qtable);
//				tick = 0;
...
//					}
				...
				System.out.println("-----------------------------");
				if (aprende) {
					best_steer_angle = qtable.getBestRewardPosition(newState);
